'''
    Reconstruction Extraction Module training code
'''
...
import re
from tools import dataloaders_reconstruction
...
def get_args():
    ...
    parser.add_argument('--input1', type=str, default='./sample_data/text_train', help='input(text image) path')
    parser.add_argument('--input2', type=str, default='./sample_data/background_train', help='input(background) path')
    parser.add_argument('--label', type=str, default='./sample_data/scene_train', help='label path')
...
if __name__ == '__main__':
    ...
    im_list = dataloaders_reconstruction.pair(input_root_dir)
    target_size = args.img_size
    dataset = dataloaders_reconstruction.SynthTextDataset(im_list, input_root_dir, input2_root_dir, label_root_dir, target_size, dataloaders_reconstruction.resize_w_pad)
    ...
    net = reconstruction_module(n_channels=7, n_classes=3)
    net.to(device)


    save_dir_input_train = './train_val_output/reconstruction_module/train/input'
    save_dir_input2_train = './train_val_output/reconstruction_module/train/input2'
    save_dir_label_train = './train_val_output/reconstruction_module/train/label'
    save_dir_output_train = './train_val_output/reconstruction_module/train/output'

    save_dir_input_val = './train_val_output/reconstruction_module/validation/input'
    save_dir_input2_val = './train_val_output/reconstruction_module/validation/input2'
    save_dir_label_val = './train_val_output/reconstruction_module/validation/label'
    save_dir_output_val = './train_val_output/reconstruction_module/validation/output'

    criterion = nn.MSELoss()
    optimizer = optim.Adam(net.parameters(), lr=0.0001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
    epochs = args.epoch

    checkpoint = './train_model/reconstruction_module'
    ...
    plt.xlabel('epoch')
    plt.title("loss")
    graph_path = './graph/reconstruction_module.png'
